			...
			switch(pc.command)
			{
				...
				case cmd_export_to_xml:
					if(base1CD->is_open())
					{
						...
						{
							if(filters->Strings[m].Length() == 0)
                            	filters->Delete(m);
							else
                            	filters->Strings[m] = String(L"^") + filters->Strings[m].UpperCase() + L"$";
						}
						...
						for(m = 0; m < k; m++)
                        	expr[m] = boost::wregex(filters->Strings[m].w_str());
				...
				case cmd_save_vendors_configs:
					if(base1CD->is_open())
					{
						...
						if (!base1CD->supplier_configs.size()) {
							mess->AddMessage_(L"Не удалось обнаружить конфигурацию поставщика. Возможно данная конфигурация снята с поддержки", msError, L"Файл", f);
						}
